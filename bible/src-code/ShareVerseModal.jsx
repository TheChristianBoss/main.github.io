...
function highlightFill(highlightColor) {
  const entry = HIGHLIGHT_COLORS[highlightColor]
...
function drawQuote(canvas, { verse, reference, translationName, theme, includeStrongs }) {
  ...
  const startY = height / 2 - blockHeight / 2 + lineHeight

  const fill = highlightFill(verse.highlightColor)
  if (fill) {
    ctx.fillStyle = fill
    ctx.globalAlpha = 0.35
    ctx.fillRect(70, startY - lineHeight * 1.05, width - 140, blockHeight + lineHeight * 0.55)
    ctx.globalAlpha = 1
  }
...
function drawPassage(canvas, { verses, reference, translationName, theme, includeStrongs }) {
  ...
  measured.forEach((item) => {
    const fill = highlightFill(item.highlightColor)
    const itemHeight = item.lines.length * lineHeight + fontSize * 0.35

    if (fill) {
      ctx.fillStyle = fill
      ctx.globalAlpha = 0.32
      ctx.fillRect(pad - 5, y - fontSize, width - pad * 2 + 10, itemHeight + fontSize * 0.55)
      ctx.globalAlpha = 1
    }
...
export default function ShareVerseModal({
  ...
  const decoratedVerses = useMemo(() => {
    return selectedVerses.map((v) => {
      const key = translationId ? verseKey(translationId, v.book, v.chapter, v.verse) : null
      return {
        ...v,
        highlightColor: key ? highlights[key] : null,
      }
    })
  }, [selectedVerses, highlights, translationId])

  const reference = rangeLabel(decoratedVerses)
  ...
  }, [fontsReady, verse, decoratedVerses, reference, translationName, theme, includeStrongs, verseHasStrongs, scope])
  ...
  return (
    <div className="share-modal-overlay" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
      <div className="share-modal" role="dialog" aria-label="Share verse as image">
        <button className="strongs-popup-close" onClick={onClose} aria-label="Close">x</button>
