...
export default function App() {
  ...
  const [view, setView] = useState('read') // 'read' | 'catalog' | 'strongs' | 'bookmarks' | 'concordance'
  ...
  const [dailyVerseShare, setDailyVerseShare] = useState(null)
  ...
  const handlePrevChapter = useCallback(() => {
    const prev = getPrevChapterRef(book, chapter)
    if (!prev) return
    setBook(prev.book)
  ...
  const handlePickConcordanceResult = useCallback(
    (r) => {
      // Prefer staying on the currently active translation if it's one of
      // the matches; otherwise jump to whichever tagged translation has it.
      const targetId = r.translationIds.includes(activeId) ? activeId : r.translationIds[0]
  ...
  const handleNavigateToVerse = useCallback((ref) => {
    setBook(ref.book)
    setChapter(ref.chapter)
    setVerseRangeInput(String(ref.verse))
    setSearchResults(null)
  }, [])
    ...
    <div className="app-shell">
      ...
      <aside className="sidebar">
        ...
        <nav className="top-nav" style={{ marginBottom: 20 }}>
          ...
          <button className={view === 'concordance' ? 'active' : ''} onClick={() => setView('concordance')}>
            Concordance
          </button>
        </nav>
        ...
        <div className="sidebar-section">
          ...
          <div className="ref-row">
            <input
              id="ref-go"
              className={`ref-input ${refError ? 'ref-input-error' : ''}`}
      ...
      <main className={`main-pane${compareTranslation ? ' main-pane-wide' : ''}`}>
        {view === 'catalog' ? (
          ...
          <ConcordancePage
            translations={sortedTranslations}
            onPick={handlePickConcordanceResult}
          />
        ) : searchResults ? (
          <SearchResults
      ...
      {dailyVerseShare && (
        <ShareVerseModal
          verse={dailyVerseShare}
          translationName={activeTranslation?.name}
          onClose={() => setDailyVerseShare(null)}
        />
      )}
    </div>
  )
